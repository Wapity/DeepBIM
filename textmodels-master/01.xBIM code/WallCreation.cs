...
namespace TxtToIFC
{
    class WallCreation
    {
        ...
        /// <returns></returns>
        static public IfcWallStandardCase CreateWall(IfcStore model, double x1, double y1, 
            double x2, double y2, double width, double height)
        {
            // define the length of the wall from the given coordinates
            double deltaX = x2 - x1;
            double deltaY = y2 - y1;
            ...
            using (var txn = model.BeginTransaction("Create Wall"))
            {
                ...
                {
                    rectProf.XDim = width;
                    rectProf.YDim = length*2;
                }
                else
                {
                    rectProf.XDim = length*2;
                    rectProf.YDim = width;
                }


                var insertPoint = model.Instances.New<IfcCartesianPoint>();
                insertPoint.SetXY(0, 0); //insert at arbitrary position       //origin of the rectangluar profile (can be anywhere)
                ...
                origin.SetXYZ(x1 + deltaX/2, y1 + deltaY/2, 0);                                     //origin of the extructed solid (future wall), this point is in the center of the wall
                ...
                wall.Representation = rep;                      // Wall representation definirion
                ...
                {
                    ax3D.RefDirection.SetXYZ(1, 0, 0);          // axe direction -> defined by the second point entrance (x2, y2)ü.
                }
                ax3D.Axis = model.Instances.New<IfcDirection>();
                ax3D.Axis.SetXYZ(0, 0, 1);                          // direction of the extrution of the wall
                ...
                //startPoint.SetXY(x1 + length, y1 + length);
        ...
        static public void AddPropertiesToWall(IfcStore model, IfcWallStandardCase wall)
        {
            using (var txn = model.BeginTransaction("Create Wall"))
            {
