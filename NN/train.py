...
from dataset import full_process_training

X_train_div, X_test_div, y1_train, y1_test, y2_train, y2_test = full_process_training()

param_grid = {'epochs':[2] , 'batch_size':[20]}
# epochs with + 100, 200 , batch_size before with  + 50, 100
#top with epochs = 200 , batch_size = 20
#update when GPU

""" Model Cooling """
model1 = KerasRegressor(build_fn = create_model_regressor_cooling , verbose = 2)
...
""" Model Heating """
model2 = KerasRegressor(build_fn = create_model_regressor_heating , verbose = 2)
