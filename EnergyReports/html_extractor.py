...
def html_single_extractor(text, get_keys=False):
    ...
    for i in range(len(text)):
        ...
        # Mean daily_range
            ...
            dict_result['Mean_daily_range'] = int(
                re.findall(r'\d+', text[i + 1])[0])
        # Area
        ...
        # Humidity
            ...
            dict_result['Relative_humidity'] = int(
                re.findall(r'\d+', text[i])[0])
        # Supply air temperature
        # first temperature for supply air temperature
        elif text[i] == 'Temperature' and index_temp == 0:
            dict_result['Supply_air_temperature'] = int(
                re.findall(r'\d+', text[i + 1])[0])
            index_temp += 1
        # Wall Area
