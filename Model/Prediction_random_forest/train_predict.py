...
""" Cooling """
grid_search_rf = MultiOutputRegressor(GridSearchCV(model, param_grid, cv= 5,return_train_score=True,verbose = 2))
grid_search_rf.fit(X_train_div, y1_train)

print('The Train R2 score for cooling load is',r2_score(y1_train, grid_search_rf.predict(X_train_div)))
print('The Test R2 score for cooling load is',r2_score(y1_test, grid_search_rf.predict(X_test_div)))
...
""" Heating """
grid_search_rf2 = MultiOutputRegressor(GridSearchCV(model, param_grid, cv= 5,return_train_score=True, verbose =2))
grid_search_rf2.fit(X_train_div, y2_train)

print('The Train R2 score for heating load is',r2_score(y2_train, grid_search_rf2.predict(X_train_div)))
print('The Test R2 score for heating load is',r2_score(y2_test, grid_search_rf2.predict(X_test_div)))
